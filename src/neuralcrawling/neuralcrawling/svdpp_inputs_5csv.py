...
def start():
    try:
        # check whether there are 8 different arguments
        if len(sys.argv) != 9:
            print("Usage: python svdpp_inputs_5csv_code.py <src filepath1> <src filepath2> <src filepath3> <src filepath4> <src filepath5> <userID> <prodID> <no of recommendation>")
            sys.exit(1)
        ...
        arg_userID = sys.argv[6]
        arg_prodID = sys.argv[7]
        arg_no_of_reco = sys.argv[8]
        
        # Select the CSV files from the source folders
        csv_files = []
        for folder in [arg_filepath1, arg_filepath2, arg_filepath3, arg_filepath4, arg_filepath5]:
