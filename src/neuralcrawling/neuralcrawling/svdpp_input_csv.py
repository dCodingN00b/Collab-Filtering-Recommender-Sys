...
def start():
    try:
        # check whether there are 4 different arguments
        if len(sys.argv) != 5:
            print("Usage: python svdpp_csv_inputs.py <src filepath> <userID> <prodID> <no of recommendation>")
            sys.exit(1)
        
        # specify filepath from argument
        arg_filepath = sys.argv[1]
        
        # specify userID and prodID from arguments
        arg_userID = sys.argv[2]
        arg_prodID = sys.argv[3]
        arg_no_of_reco = sys.argv[4]
        
        # Select the CSV files from the source folders
        csv_files = []
        for folder in [arg_filepath]:
