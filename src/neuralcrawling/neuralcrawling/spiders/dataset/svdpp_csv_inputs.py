...
def svdppcv3_gs(df, reader, rng):    
    ...
    norm_pred = svdppalgo.predict(uid='AHBIAXMBYHSCAWMIDYNU6VGBRO7Q', iid='B082YF43GQ', r_ui=None).est
    
    print ("\nrating prediction for AHBIAXMBYHSCAWMIDYNU6VGBRO7Q on prodID = B082YF43GQ: ", round(norm_pred))
        
    print ("\n============ Tuned svdpp ============\n")
    
    # hyperparameters # doesn't change with 10 and 30
    # maybe tweak this init_std_dev: [0.05, 0.1, 0.15] # default 0.1
    param_grid = {"n_factors": [15, 20], 
                  "n_epochs": [20, 40, 60], #30, 50, #65 best for now
                  "lr_all": [0.002, 0.004, 0.007], #
                  "reg_all": [0.02, 0.04, 0.07] #0.07
                  } 
    ...
    if (rmseVal_cv>rmseVal_ori):
        ...
        # print out the rounded values for the userID and prodID
        print ("\nrating prediction for AHBIAXMBYHSCAWMIDYNU6VGBRO7Q on prodID = B082YF43GQ: ", round(norm_pred))
        
        print ("\nrandom prediction:")
        #pass df and svdalgo into rand_pred
        rand_pred(df, svdppalgo)
        
        # request top 5 prodID to be recommended 
        # pass userID, dataframe, train_set, algo and n_count of top items
        topReco_prod = similar_products('B082YF43GQ', df, train_set, svdppalgo, 5)
        
        print ("\nRecommendation for 'B082YF43GQ': ")
    ...
    # compare if rmseVal_ori more than rmseVal_cv, we will use tuned SVD++ parameters and run the prediction 
        ...
        pred_cv3 = svd_pp_cv3.predict(uid='AHBIAXMBYHSCAWMIDYNU6VGBRO7Q', iid='B082YF43GQ', r_ui=None).est
        
        print ("\nrating prediction for AHBIAXMBYHSCAWMIDYNU6VGBRO7Q on prodID = B082YF43GQ: ", round(pred_cv3))
        ...
        # request top 5 prodID to be recommended 
        # pass userID, dataframe, train_set, algo and n_count of top items
        topReco_prod = similar_products('B082YF43GQ', df, train_set, svd_pp_cv3, 5)
