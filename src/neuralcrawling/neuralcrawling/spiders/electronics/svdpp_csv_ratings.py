...
def start():
    try:
        if len(sys.argv) != 3:
            print("Usage: python svdpp_csv_ratings.py <userID> <prodID>")
            sys.exit(1)
        
        arg_userID = sys.argv[1]
        arg_prodID = sys.argv[2]
        
        #read in data from .csv file
        df = pd.read_csv('electronics_out.csv')
        ...
        #svd++ algo
        print ("\n================== svdpp algo ==================\n")
        svdppcv3_gs(df, reader, rng, arg_userID, arg_prodID)
...
def svdppcv3_gs(df, reader, rng, arg_userID, arg_prodID):    
    ...
    if (rmseVal_cv>rmseVal_ori):
        ...
        print (f"rating prediction for {arg_userID} on prodID = {arg_prodID}: {round(norm_pred)}")
    ...
    # compare if rmseVal_ori more than rmseVal_cv, we will use tuned SVD++ parameters and run the prediction 
        ...
        # use inputs from 
        pred_cv3 = svd_pp_cv3.predict(uid=arg_userID, iid=arg_prodID, r_ui=None).est
        
        #
        print (f"rating prediction for {arg_userID} on prodID = {arg_prodID}: {round(pred_cv3)}")
