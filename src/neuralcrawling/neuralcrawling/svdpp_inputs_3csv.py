...
def start():
    try:
        # check whether there are 6 different arguments
        if len(sys.argv) != 7:
            print("Usage: python svdpp_inputs_3csv.py <src filepath1> <src filepath2> <src filepath3> <userID> <prodID> <no of recommendation>")
            sys.exit(1)
        ...
        arg_userID = sys.argv[4]
        arg_prodID = sys.argv[5]
        arg_no_of_reco = sys.argv[6]
        ...
        csv_files = []
        for folder in [arg_filepath1, arg_filepath2, arg_filepath3]:
