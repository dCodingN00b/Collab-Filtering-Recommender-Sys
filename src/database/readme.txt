To run the program, firstly download XAMPP, which is a popular PHP development environement.
It can be found under https://www.apachefriends.org/ , download based on the operating system you are using.

After setting up and installing XAMPP, open the XAMPP control panel and start Apache and MySQL.
Thereafter, go your browser and type in localhost, you should be directed to http://localhost/dashboard/ 

To set up the database:

Step 1: make sure you are in localhost
Step 2: Type in localhost/CreateDB.php
Step 3: Type in localhost/CreateTable.php
Step 4: Type in addDummyRecords.php

Your database will set up.

Lastly, to begin the program, run localhost/default.php
